    ...
    for step, data in enumerate(dataloader, 0):
        ...
        if step > 0 and step % UPDATE_INTERVAL == 0:
            ...
            if img_set is not None:
                ...
                fullpath = '%s/attention_maps%d_epoch%d.png' % (image_dir, step, epoch)
                im.save(fullpath)
    return count
...
if __name__ == "__main__":
    ...
    mkdir_p(model_dir)
    mkdir_p(image_dir)
    writePath = output_dir + '/Model/results.txt'
    ...
    try:
        ...
        for epoch in range(start_epoch, cfg.TRAIN.MAX_EPOCH):
            ...
            if len(dataloader_val) > 0:
                ...
                epochLossLr = str(epoch) + ' ' + str(s_loss.item()) + ' ' + str(w_loss.item()) + ' ' + str(lr)
                with open(writePath, 'a') as f:
                    f.write(epochLossLr + "\n")
            print('-' * 89)
            if lr > cfg.TRAIN.ENCODER_LR/1000.:
                lr *= 0.98
